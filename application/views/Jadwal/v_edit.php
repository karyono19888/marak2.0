		...
		<div class="content-header row">
			<div class="content-header-left col-md-9 col-12 mb-2">
				<div class="row breadcrumbs-top">
					<div class="col-12">
						<h2 class="content-header-title float-start mb-0">Edit Jadwal</h2>
						<div class="breadcrumb-wrapper">
							<ol class="breadcrumb">
								<li class="breadcrumb-item"><a href="<?= base_url('Jadwal'); ?>">Jadwal</a>
								</li>
								<li class="breadcrumb-item active">Edit Jadwal Kunjungan
								</li>
							</ol>
						</div>
					</div>
				...
				<div class="col-lg-9">
					<div class="card">
						<div class="card-header">
							<h4 class="card-title">Edit Rencana Kunjungan</h4>
						</div>
						<div class="card-body">
							<div class="row custom-options-checkable g-1">
								<input type="hidden" id="new_or_update" value="<?= $data['new_or_update']; ?>">
								<input type="hidden" id="id_jadwal" value="<?= $data['id_jadwal']; ?>">
								<div class="col-md-6">
									<input class="custom-option-item-check" <?= $data['new_or_update'] == "0" ? 'checked' : '' ?> hidden type="radio" name="kunjungan_baru" id="kunjungan_baru" />
								...
								<div class="col-md-6">
									<input class="custom-option-item-check" <?= $data['new_or_update'] == "1" ? 'checked' : '' ?> type="radio" name="kunjungan_update" id="kunjungan_update" value="" />
...
<script>
	$(document).ready(function() {
		let new_or_update = $('#new_or_update').val();
		let id = $('#id_jadwal').val();

		if (new_or_update == 0) {
			$.ajax({
				type: "POST",
				url: "<?= site_url('Jadwal/EditKunjunganBaru') ?>",
				data: {
					id: id
				},
			...
			$.ajax({
				type: "POST",
				url: "<?= site_url('Jadwal/EditKunjunganUpdate') ?>",
				data: {
					id: id
				},
				success: function(response) {
